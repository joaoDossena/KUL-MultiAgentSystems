...
/**
 * This class represents the implementation of an Agent in the MAS. It interacts with the Environment for new information of the world by running a separate
 * thread. The agent implementation contains the local view on the world and any received messages. It also has a behavior.
 */
public abstract class AgentImp extends ActiveImp implements AgentState, AgentCommunication, AgentAction {
    ...
    /**
     * The memory of an agent has the form of a key mapped to a memory fragment (represented as String) e.g.  "target" -> "3, 4"
     */
    private Map<String, AgentMemoryFragment> memory;
     ...
     */
    protected AgentImp(ActiveItemID id, EventBus eventBus) {
        super(id);
    ...
    /**
     * Create a mail from this AgentImp to <receiver> and with message <message> and add the resulting mail to the buffer of outgoing mails.
     *
    ...
    /**
     * Put a packet down on the given Coordinate. If the cell contains a destination, the packet will be delivered. If the cell is empty, the packet will be put
     * down.
     *
    ...
    private void pickGeneratorPacket(PacketGenerator generator) {
        ...
        this.concludeWithCondition(this.hasSufficientEnergyDefault(),
                this.generateActionOutcome(
                        new InfPickGeneratorPacket(getEnvironment(), generator.getX(), generator.getY(), getActiveItemID(), generatorColor)));
    }
     ...
     */
    @Override
    public void addMemoryFragment(String key, AgentMemoryFragment data) {
     ...
     */
    @Override
    public AgentMemoryFragment getMemoryFragment(String key) {
    ...
    /**
     * Triggers an update of the behavior. If necessary the agent changes behavior.
     */
    ...
    /**
     * Returns a copy of the Agent Item which represents this AgentImp in the Environment
     *
